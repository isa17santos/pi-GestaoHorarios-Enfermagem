...
const texts = computed(() => ({
  title: currentLocale.value === 'pt' ? 'Novo Tipo de Turno' : 'New Shift Type',
  subtitle: currentLocale.value === 'pt' ? 'Preencha os dados do novo tipo de turno' : 'Fill in the new shift type details',
  ...
  save: currentLocale.value === 'pt' ? 'Criar Tipo de Turno' : 'Create Shift Type',
  saving: currentLocale.value === 'pt' ? 'A guardar...' : 'Saving...',
  success: currentLocale.value === 'pt' ? 'Tipo de turno criado com sucesso!' : 'Shift type created successfully!',
  error: currentLocale.value === 'pt' ? 'Erro ao criar o tipo de turno.' : 'Error creating shift type.',
  validation: {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await $fetch(`${config.public.apiBase}/shift-types`, {
      method: 'POST',
    ...
    console.error('Erro ao guardar tipo de turno:', err)
    ...
    if (err?.statusCode === 422 && err?.data?.errors) {
      ...
      if (serverErrors.name?.length) {
        ...
        errors.value.name = /taken|exists|unique|já existe/.test(firstNameError)
          ? 'name_taken'
...
onMounted(async () => {
  ...
  // Apenas administradores podem criar/editar tipos de turno
...
<style scoped>
/* Coluna cheia para o campo Nome */
.stc-full-width {
  grid-column: span 2;
}

/* Input de hora: remove setinhas nativas nos browsers Webkit */
...
/* Input nativo de cor escondido, ocupa todo o swatch */
