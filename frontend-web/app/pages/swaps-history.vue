...
onMounted(async () => {
  ...
  const role = currentUser.value?.role?.toLowerCase().trim().replace(' ', '_') || ''
...
<template>
  <main class="dashboard-layout sw-page">
    ...
    <div ref="pdfPageRef" style="display:none; width:1200px; padding:45px 50px; background:#fff; font-family:Arial,sans-serif; color:#222; box-sizing:border-box;">

      <!-- Header — same pattern as schedule-view.vue -->
      <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #f1f5f9; padding-bottom:15px; margin-bottom:25px;">
        <img src="/images/logotipo.png" alt="ShiftCare" style="height:42px;" />
        <div style="text-align:right;">
          <h2 style="margin:0; font-size:1.4rem; font-weight:800; color:#0f172a;">{{ texts.pdf.title }}</h2>
          <span style="font-size:0.95rem; font-weight:600; color:#64748b; display:block; margin-top:2px;">
            {{ selectedNurseName !== texts.allNurses ? selectedNurseName : (currentLocale === 'pt' ? 'Todos os enfermeiros' : 'All nurses') }}
            <template v-if="selectedMonth"> — {{ currentLocale === 'pt' ? 'Mês de' : 'Month of' }} {{ selectedMonthLabel }}</template>
          </span>
        </div>
      </div>
      ...
      <table style="width:100%; border-collapse:collapse; font-size:13px;">
        <thead>
          <tr style="background:#f8fafc; color:#0f172a;">
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.colDate }}</th>
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.colNurseA }}</th>
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.colShiftA }}</th>
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.colNurseB }}</th>
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.colShiftB }}</th>
            <th style="padding:10px 12px; text-align:left; border-bottom:2px solid #e2e8f0; font-weight:700; color:#64748b;">{{ texts.pdf.signature }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#334155;">{{ row.date }}</td>
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#0f172a; font-weight:600;">{{ row.nurseA }}</td>
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#334155;">{{ row.shiftA }}</td>
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#0f172a; font-weight:600;">{{ row.nurseB }}</td>
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; color:#334155;">{{ row.shiftB }}</td>
            <td style="padding:10px 12px; border-bottom:1px solid #e2e8f0; min-width:140px; color:#94a3b8;">___________</td>
          </tr>
        </tbody>
      </table>

      <!-- Footer -->
      <div style="margin-top:32px; padding-top:14px; border-top:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center; font-size:11px; color:#94a3b8;">
        <span>ShiftCare</span>
        <span>{{ texts.pdf.printed }}: {{ new Date().toLocaleDateString(currentLocale === 'pt' ? 'pt-PT' : 'en-US') }}</span>
      </div>
    </div>
  </main>
</template>
