...
const texts = computed(() => ({
  title: currentLocale.value === 'pt' ? 'Atribuir Baixa Médica' : 'Assign Medical Leave',
  subtitle: currentLocale.value === 'pt' ? 'Registe o período de ausência médica de um profissional' : 'Register the medical absence period for a professional',
  ...
  save: currentLocale.value === 'pt' ? 'Gravar Baixa Médica' : 'Save Medical Leave',
  saving: currentLocale.value === 'pt' ? 'A guardar...' : 'Saving...',
  success: currentLocale.value === 'pt' ? 'Baixa médica registada com sucesso!' : 'Medical leave registered successfully!',
  error: currentLocale.value === 'pt' ? 'Erro ao registar baixa médica.' : 'Error registering medical leave.',
  validation: {
...
// Procura os enfermeiros e enfermeiros chefes ativos
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    await $fetch(`${config.public.apiBase}/medical-leaves`, {
      method: 'POST',
      headers: {
    ...
    console.error('Create Medical Leave Error:', err)
...
<template>
  <main class="dashboard-layout user-create-page">
    ...
    <section class="uc-card">
      <div v-if="fetchingNurses" class="loading-state">
